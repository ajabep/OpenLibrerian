...
?></h1><?php
	...
	if( count($objectsList) ) {
		...
		foreach( $objectsList as $object ){
			?><div class="object"><div class="objectContent"><h2><?php echo $object->name(); ?></h2><p class="description"><?php echo $object->description(); ?></p><p class="readMore pull-right"><a href="<?php echo PREFIX_LINK_LANG . 'object/' . $object->ID() . '/' . stringToUrl( $object->name() ) . '/'; ?>">Go &rarr;</a></p><?php
			if( $isAdmin ) {
				echo '<p class="editLink pull-left"><a href="' . PREFIX_LINK_LANG . 'object/' . $object->ID() . '/' . stringToUrl( $object->name() ) . '/">Editer &rarr;</a></p><form role="form" method="post" action="' . PREFIX_LINK_LANG . 'object/' . $object->ID() . '/' . stringToUrl($object->name()) . '/delete/" class="deleteLink text-center"><input type="hidden" aria-hidden="true" name="token" value="{{tokenDeleteObject}}"><button type="submit" role="button" class="btn btn-link">Supprimer &rarr;</button></form>';
			}
			?></div></div><?php
		}
		?></div><?php
		...
		?><p class="alert alert-danger">Il n'y a rien ici !<br>Nada. Nothing. Zip. Zilch. Zero.</p><?php
	}
?>