...
class scssc {
	...
	public function compile($code, $name = null)
	{
		...
		switch( $formatter ){
			...
			case 'scss_formatter_compressed' :
				$this->formatter = new scss_formatter_compressed();
				break;
	...
	protected function compileChild($child, $out) {
		...
		case "assign":
			...
			break;
		case "comment":
			if( $this->formatter->displayComment )
				$out->lines[] = $child[1];
			break;
		case "mixin":
		case "function":
...
$this->peek("(.*?)(\\n|$)", $m, $count);
var_dump("$msg: failed at `$m[1]` $loc"); exit;
